...
class HybridTrainPipe(Pipeline):
    def __init__(self, batch_size, num_threads, device_id, data_dir, crop, dali_cpu=False):
        ...
        self.input = ops.TFRecordReader(path = data_dir + ".tfrecord",
    ...
    def define_graph(self):
        rng = self.coin()
        input = self.input(name="Reader")
        self.jpegs = input["image"]
...
def main():
    ...
    # Data loading code
    if len(args.data) == 1:
        traindir = os.path.join(args.data[0])
        valdir = os.path.join(args.data[0])
    else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #         print('##Top-1 {0}\n'
        #               '##Top-5 {1}\n'
        #               '##Perf  {2}'.format(0, 0, args.total_batch_size / total_time.avg))
